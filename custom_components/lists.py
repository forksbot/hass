...
SERVICE_DELETE_LIST_SCHEMA = vol.Schema({
    vol.Required(ATTR_LIST_ID): vol.Any(None, cv.string) #TODO Cannot delete 'Inbox'
})

SERVICE_DELETE_CATEGORY_SCHEMA = vol.Schema({
    vol.Required(ATTR_CATEGORY_ID): vol.Any(None, cv.string), #TODO Cannot delete 'Uncategorized'
    vol.Optional(ATTR_LIST_ID, default='0'): vol.Any(None, cv.string)
})
