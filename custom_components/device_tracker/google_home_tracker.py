"""
Tracking of bluetooth devices using Google Home hardware.

For more details about this platform, please refer to the documentation at
https://home-assistant.io/components/device_tracker.google_home_tracker/
"""
import logging
...
REQUIREMENTS = ['google_home_config==0.38']

BT_PREFIX = 'BT_'

CONF_REQUEST_RSSI = 'request_rssi'
CONF_HTTP_ADDRESS = 'ip_address'
...
def setup_scanner(hass, config, see, discovery_info=None):
    """Set up the Google Home Scanner."""
    # pylint: disable=import-error
    import google_home_config
    
    # Initialize with address from config
    google_home_config.__init__(config.get(CONF_HTTP_ADDRESS))
    ...
    def discover_devices():
        ...
        google_home_config.do_bt_scan(8)
        sleep(8)
        result = google_home_config.get_bt_scan_results()
        _LOGGER.debug("Bluetooth devices discovered = %d", len(result))
        # Need to parse into unique devices
        return result
