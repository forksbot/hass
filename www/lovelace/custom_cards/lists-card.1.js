class ListsCard extends HTMLElement {
    ...
    <app-header-layout has-scrolling-region>
      <app-header slot="header" fixed>
        <app-toolbar>
          ...
          <div main-title>[[localize('panel.lists')]]</div>
  ...
  connectedCallback() {
    ...
    this.hass.connection.subscribeEvents(this._fetchData, 'lists_updated')
  ...
  _itemCompleteTapped(ev) {
    ev.stopPropagation();
    this.hass.callApi('post', 'lists/item/' + ev.model.item.id, {
      complete: ev.target.checked
    }).catch(() => this._fetchData());
  }

  _addItem(ev) {
    this.hass.callApi('post', 'lists/item', {
  ...
  _saveEdit(ev) {
    ...
    this.set(['lists', index, 'name'], name);
    this.hass.callApi('post', 'lists/item/' + item.id, {
      name: name
    }).catch(() => this._fetchData());
  }

  _clearCompleted() {
    this.hass.callApi('POST', 'lists/clear_completed');
  }
}
